...
const ScaleButton = styled.button<{ $isSelected: boolean }>`
  ...
  background-color: ${(props) =>
    props.$isSelected ? (props.theme as EventureTheme).colors.primary : 'white'};
  color: ${(props) =>
    props.$isSelected ? 'white' : (props.theme as EventureTheme).colors.gray.gray700};
  ...
  &:hover {
    ...
    background-color: ${(props) =>
      props.$isSelected
        ? (props.theme as EventureTheme).colors.primary
        : `${(props.theme as EventureTheme).colors.gray.mainColorLight}15`};
...
type ScaleQuestion = QuizQuestionBase & {
  scaleMin?: number;
  scaleMax?: number;
...
interface Props {
  question: QuizQuestionBase | ScaleQuestion;
...
export function QuizStepScale({ question, currentAnswer, onAnswer }: Props) {
  ...
  const hasScale =
    scaleQuestion.scaleMin !== undefined ||
    scaleQuestion.scaleMax !== undefined ||
    scaleQuestion.scaleLabels !== undefined;

  const min = hasScale ? (scaleQuestion.scaleMin ?? 0) : 0;
  const max = hasScale ? (scaleQuestion.scaleMax ?? 10) : 10;
  ...
  return (
    <ScaleContainer>
      ...
      {hasScale && scaleQuestion.scaleLabels && (
        <ScaleLabels>
          <span>{scaleQuestion.scaleLabels.min}</span>
          <span>{scaleQuestion.scaleLabels.max}</span>
        </ScaleLabels>
      )}
