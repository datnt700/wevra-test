...
export const Styled = {
  TaskCard: styled.div<{ $completed: boolean }>`
    ...
    border: 2px solid
      ${(props) => {
        const theme = props.theme as EventureTheme;
  ...
  Checkbox: styled.div<{ $completed: boolean }>`
    ...
    background: ${(props) =>
      props.$completed ? (props.theme as EventureTheme).colors.success : 'transparent'};
    display: flex;
    align-items: center;
  ...
  XPBadge: styled.span`
    ...
    background: ${(props) => (props.theme as EventureTheme).colors.successLight}20;
    border-radius: 4px;
  `,
