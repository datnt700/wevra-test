...
Users progress through stages like:- ✅ **@eventure/eventured** UI component
library with Emotion styling
...
| UI Components      | @eventure/eventured (Emotion + Radix UI)        |
...
Wevra uses **@eventure/eventured** - a comprehensive component library with 60+ components:1. **Package name**: Update `name` in `package.json` to `@tavia/your-app-name`
...
- [ ] Community features- This template uses `@eventure/eventured` for UI components
...
- [@eventure/eventured Components](../../packages/taviad/README.md)
