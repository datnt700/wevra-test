# Frontoffice - Customer Restaurant Discovery App

Customer-facing application for browsing and discovering restaurants.
...
**IMPORTANT:** Frontoffice shares the database with backoffice.
...
# Or use the automated setup from root
pnpm db:setup
```

**Do NOT run** `pnpm docker:up` in frontoffice - the docker-compose is disabled
...
Both apps point to the same database for real-time data synchronization.
...
User visits page
    ↓
useSearchRestaurants() hook (React Query)
    ↓
searchRestaurantsAction() (Server Action)
    ↓
...
- `src/actions/restaurant.actions.ts` - Server actions for data fetching
- `src/hooks/useRestaurants.ts` - React Query hooks
- `src/services/restaurant.service.ts` - HTTP service layer
- `src/lib/prisma.ts` - Prisma client singleton
- `src/types/restaurant.ts` - TypeScript types
...
```bash
# 1. Ensure backoffice database is running
cd apps/backoffice
pnpm docker:up
...
3. **Verify React Query DevTools** (bottom-right corner) shows cached data

### Different data than backoffice

Both apps use the same database. If you see different data:

1. Check both `.env` files have identical `DATABASE_URL`
2. Clear browser cache and React Query cache
...
```bash
# Update package.json dev script to use different port
"dev": "next dev --turbopack --port 3004"
```

## 🌐 Internationalization

Uses **modular i18n** with next-intl.
...
- `common.json` - App name, general strings
- `navigation.json` - Menu items
- `home.json` - Restaurant-specific content
- `actions.json` - Buttons (save, cancel, etc.)
...
export default function Example() {
  return (
    <Card>
      <Input label="Search" placeholder="Find restaurants..." />
...
- **Backoffice** (port 3000) - Restaurant management dashboard
- **Analytics** (port 3001) - Event tracking API (Fastify)
...
frontoffice/  internationalization (i18n) with next-intl
...
### Generate New Modules│   ├── layout.tsx          # Root layout with providersfrontoffice/
