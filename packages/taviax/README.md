...
> React Native design system for Tavia mobile applications ("X" for
> cross-platform)

A comprehensive React Native component library that shares design tokens **and
styling approach** with `@tavia/taviad` for true cross-platform consistency.

## Features

- ✅ **@emotion/native**: Same styling library as `@tavia/taviad` (web)
- ✅ **Shared Design Tokens**: Same colors, spacing, and typography as
  `@tavia/taviad`
- ✅ **Unified API**: Component props and patterns match `@tavia/taviad`
...
# Install @tavia/taviax
pnpm add @tavia/taviax
```
...
2. **Unified API**: Component APIs match `@tavia/taviad` exactly where possible
...
| Aspect              | @tavia/taviad (Web)  | @tavia/taviax (Mobile) |
| ------------------- | -------------------- | ---------------------- |
...
**Key Benefit**: Developers can switch between web (`@tavia/taviad`) and mobile
(`@tavia/taviax`) with minimal code changes, as both use the same Emotion
styling approach and component APIs.
