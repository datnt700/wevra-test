...
// Suppress expected TipTap/ProseMirror errors in jsdom environment
beforeAll(() => {
  // Handle uncaught exceptions (TipTap/ProseMirror synchronous errors)
  const originalConsoleError = console.error;
  console.error = (...args: any[]) => {
    const message = args.join(' ');
    // Suppress known TipTap/ProseMirror errors that don't affect test validity
    if (
  ...
  // Handle unhandled promise rejections (TipTap "image" node error)
  ...
  // Handle uncaught errors (TipTap/ProseMirror synchronous errors)
