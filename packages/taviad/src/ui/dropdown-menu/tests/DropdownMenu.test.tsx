...
describe('DropdownMenu', () => {
  describe('Basic Rendering', () => {
    it('renders trigger element', () => {
      render(<DropdownMenu trigger={<button>Open Menu</button>} items={[{ label: 'Item 1' }]} />);
  ...
  describe('Menu Items', () => {
    it('calls onSelect when item is clicked', async () => {
      ...
      render(
        <DropdownMenu
          trigger={<button>Menu</button>}
          items={[{ label: 'Action', onSelect: handleSelect }]}
        />
      );
    ...
    it('renders disabled items', async () => {
      ...
      render(
        <DropdownMenu
          trigger={<button>Menu</button>}
          items={[{ label: 'Disabled Item', isDisabled: true, onSelect: handleSelect }]}
        />
      );
    ...
    it('uses custom key prop when provided', async () => {
      ...
      render(
        <DropdownMenu
          trigger={<button>Menu</button>}
          items={[{ key: 'custom-key', label: 'Custom Key Item' }]}
        />
      );
  ...
  describe('Custom Trigger', () => {
    it('renders custom trigger element', () => {
      render(<DropdownMenu trigger={<div>Custom Trigger</div>} items={[{ label: 'Item' }]} />);
  ...
  describe('Interaction', () => {
    it('closes menu after item is selected', async () => {
      const user = userEvent.setup();
      render(<DropdownMenu trigger={<button>Menu</button>} items={[{ label: 'Close Me' }]} />);
  ...
  describe('Edge Cases', () => {
    ...
    it('handles missing onSelect gracefully', async () => {
      const user = userEvent.setup();
      render(<DropdownMenu trigger={<button>Menu</button>} items={[{ label: 'No Handler' }]} />);
