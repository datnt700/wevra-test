...
export const Styled = {
  ...
  Marker: styled.div<{ $isAvailable?: boolean }>`
    ${({ theme, $isAvailable }) => {
      const taviaTheme = theme as TaviaTheme;
      const markerColor = $isAvailable ? taviaTheme.colors.success : taviaTheme.colors.danger;
      return `
        display: flex;
        align-items: center;
  ...
  Popup: styled.div`
    ${({ theme }) => {
      ...
      return `
        ...
        .status.available {
          background-color: ${taviaTheme.colors.gray.colorSuccessLight};
          color: ${taviaTheme.colors.success};
        }

        .status.unavailable {
          background-color: ${taviaTheme.colors.gray.colorDangerLight};
