...
const List = styled(RadixTabs.List)`
  ${({ theme }) => {
    ...
    return `
      ...
      display: flex;
      gap: 0.5rem;
      border-bottom: 1px solid ${eventureTheme.colors.gray.gray200};

      &[data-orientation='vertical'] {
        flex-direction: column;
        border-bottom: none;
        border-right: 1px solid ${eventureTheme.colors.gray.gray200};
      }
    `;
  }}
`;
...
const Trigger = styled(RadixTabs.Trigger)`
  ${({ theme }) => {
    ...
    return `
      ...
      user-select: none;
      border: 0;
      border-radius: ${eventureTheme.radii.sm} ${eventureTheme.radii.sm} 0 0;
      transition: all 0.2s ease;
      position: relative;

      &[data-orientation='vertical'] {
        border-radius: ${eventureTheme.radii.sm} 0 0 ${eventureTheme.radii.sm};
      ...
      &[data-state='active'] {
        color: ${eventureTheme.colors.primary};
        border-bottom: 2px solid ${eventureTheme.colors.primary};

        &[data-orientation='vertical'] {
          border-bottom: none;
          border-right: 2px solid ${eventureTheme.colors.primary};
        }
      }
